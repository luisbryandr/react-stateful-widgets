...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    const featuredPro = programmers.find(dev=> dev.id === activeProgrammer)
    return featuredPro.name
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setActiveProgrammer(dev.id)/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
